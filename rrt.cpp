...
double RRT::getCost(int nodeID)
{
    return rrtTree[nodeID].cost;
}
...
void RRT::setCost(int nodeID, double input_cost)
{
    rrtTree[nodeID].cost = input_cost;
}
...
* returns euclidean distance between two set of X,Y coordinates
*/
double RRT::distance(rrtNode source, rrtNode destination){
    return sqrt(pow(destination.posX - source.posX,2) + pow(destination.posY - source.posY,2) + pow(destination.posZ - source.posZ,2));
}
